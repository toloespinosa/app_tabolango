...
document.addEventListener('DOMContentLoaded', () => {
    const btnPermiso = document.getElementById('btn-permiso-notis');
    ...
    btnPermiso.onclick = async () => {
        ...
        // 2. Si ya tiene permiso, avisar al usuario
        if (Notification.permission === 'granted') {
            return Swal.fire({
                ...window.swalConfig,
                icon: 'info',
                title: 'Ya activado',
                text: 'Las notificaciones ya están habilitadas en este dispositivo.'
            });
        }

        // 3. Solicitar permiso con una animación de carga
        ...
        if (resultado === 'granted') {
            Swal.fire({
                ...
                icon: 'success',
                title: '¡Excelente!',
                text: 'Ahora recibirás alertas de tus pedidos en este equipo.',
                confirmButtonText: 'Genial'
            });

            // 🔥 AQUÍ GATILLAMOS LA GENERACIÓN DEL TOKEN
            // Si tienes una función para registrar el token en la BD, llámala aquí:
            if (typeof window.actualizarTokenFCM === 'function') {
                window.actualizarTokenFCM();
            }
            ...
            Swal.fire({
                ...
                html: 'Has bloqueado las notificaciones. Para activarlas, haz clic en el <b>candado 🔒</b> junto a la URL y cambia el ajuste a "Permitir".'
            });
        }
    };
});